...
const CONFIG = {
  ...
  gameItemsFile: path.join('json', 'gameItems.json'),
  statisticsFile: path.join('json', 'statistics.json'),
};
...
class ServerSetup {
  constructor() {
    try {
      ...
      // Определяем пути к файлам данных (gameItems.json и statistics.json)
      if (this.isPkg) {
        ...
        // В режиме разработки: пробуем build/json/ (после сборки), затем public/json/ (исходники)
  ...
  async initializeDataDir() {
    try {
      ...
      console.log(`📂 Проверка файла gameItems.json: ${gameItemsFile}`);
      console.log(`📂 Файл существует: ${gameItemsExists}`);
      console.log(`📂 Проверка файла statistics.json: ${statisticsFile}`);
  ...
  logServerInfo() {
    ...
    console.log(`📁 gameItems.json: ${this.gameItemsFile}`);
